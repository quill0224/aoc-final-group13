...
module MC #(
    ...
    always_comb begin
        if (reg_length == 16'd0)       target_words = 3'd0;
        else if (reg_length <= 16'd4)  target_words = 3'd1;
        else if (reg_length <= 16'd8)  target_words = 3'd2;
        else if (reg_length <= 16'd12) target_words = 3'd3;
        ...
        case (cs)
            ...
            ST_SEND_CFG:  begin
                if (pe_cfg_ready) begin
                    if (reg_length == 16'd0) ns = ST_NEXT_PKT;
                    else                     ns = ST_REQ_DATA;
                end
            end
            ...
            ST_NEXT_PKT:  begin
                if (pkt_cnt == reg_pkt_max - 1) ns = ST_DONE;
                else                            ns = ST_REQ_HDR;
            end
            ST_DONE:      ns = ST_IDLE;
            default:      ns = ST_IDLE;
    ...
    logic unused_ok;
    assign unused_ok = &{1'b0, mc_mode, mc_glb_base_B, 1'b0};
